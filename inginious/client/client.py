...
class Client(BetterParanoidPirateClient):
    ...
    def new_job(self, priority, job_info, inputdata, callback, launcher_name="Unknown", debug=False, ssh_callback=None):
        ...
        : param job_info: a dict containing all the needed information about the job (course, task, environment type, environment, job image)
        : type job_info: dict
        :param inputdata: input from the student
        :type inputdata: Storage or dict
        :param callback: a function that will be called asynchronously in the client's process, with the results.
        ...
        if job_info.environment_type not in self._available_environments or job_info.environment not in self._available_environments[job_info.environment_type]:
            self._logger.warning("Env %s/%s not available for task %s/%s", job_info.environment_type, job_info.environment, course.get_id(),
                                 task.get_id())
        ...
        msg = ClientNewJob(job_id, priority, course.get_id(), task.get_id(), task.get_problems_dict(), inputdata,
                           job_info.environment_type, job_info.environment, environment_parameters, debug, launcher_name)
